  ...
  return (
    <div className='min-h-screen flex items-center justify-center'>
        <form onSubmit={handleSignIn} className='max-w-md w-full px-6'>
            <h2 className='font-bold pb-2 text-center text-2xl'>Iniciar sesion</h2>
            <p className='text-center mb-4'>No tienes una cuenta? <Link to="/signup">Crea una!</Link></p>
            <div className='flex flex-col py-4'>
                <input onChange={(e) => setEmail(e.target.value)} className='p-3 mt-6 text-white' style={{backgroundColor: '#0D0D0D'}} type="email" name='email' id='signin-email' placeholder='Email'/>
                <input onChange={(e) => setPassword(e.target.value)} className='p-3 mt-6 text-white' style={{backgroundColor: '#0D0D0D'}} type="password" name='password' id='signin-password' placeholder='Password'/>
